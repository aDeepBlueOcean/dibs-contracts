...
contract DibsRandomSeedGenerator is
    VRFConsumerBaseV2,
    ConfirmedOwner,
    AccessControl
{
    ...
    struct SeedRequestStatus {
        ...
        uint32 roundId; // the roundId of the request
    ...
    mapping(uint32 => uint256) public roundToRequestId; // roundId => requestId
    mapping(uint32 => uint256) public roundToSeed; // roundId => seed
    ...
    constructor(
        ...
        bytes32 keyHash_
    ) VRFConsumerBaseV2(VRFCoordinator_) ConfirmedOwner(admin_) {
    ...
    event SeedRequested(uint32 roundId, uint256 requestId);
    ...
    function requestRandomSeed(
        ...
        // round must be over
        if (IDibsLottery(dibsLottery).getActiveLotteryRound() <= roundId) {
    ...
    event SeedFulfilled(uint32 roundId, uint256 seed);
    ...
    /// @notice Set the dibsLottery address
    /// @param dibsLottery_ The address of the dibsLottery contract
    function setDibsLottery(address dibsLottery_) external onlyRole(SETTER) {
        emit SetDibs(dibsLottery, dibsLottery_);
    ...
    function setKeyHash(bytes32 keyHash_) external onlyRole(SETTER) {
